...
exports.create = function(req, res) {
	...
	} if (!newItem.nom || !newItem.prenom || !newItem.login  || !newItem.password ) {		
		return res.status(400).send('One or more required fields are missing')
	} else {		
		Item.create(newItem, function(err, idutilisateur) {
			console.log('err: ', err);
			//if (err === Object) res.status(500).send('Item already exist with name ' + err.item_name);
			
			if (err || idutilisateur <= 0) return res.status(500).send('Error occured during saving item');
